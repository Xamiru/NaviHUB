...
- Artist and album pages may remember one matching public Spotify source in nullable, unique `music_artist.spotify_id` / `music_album.spotify_id`. Entity previews live only in an eight-entry, 30-minute process cache; expiry or restart reinspects the remembered URL. A confirmed name mismatch is one-shot only and must never write either source column. After scans, source IDs may auto-link only when every source track resolves strictly to one unambiguous local entity.
...
`src/main/index.ts` — **thirteen calls, and the order is load-bearing**:

```
settleAllTasksOnQuit
→ killActiveMusicDownload → abortActiveCoachTurn → killActiveUpdate → killActivePrepare
→ killActiveOcr → killSqlSandbox → stopAchievementWatcher → finalizeActiveGameSession
