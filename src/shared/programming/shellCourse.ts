...
\`\`\``,
      questions: [
        ...
        },
        {
          ...
          options: [
            'It starts faster because env caches the interpreter path',
            'It finds bash via $PATH, so it works where bash is not at /bin/bash (macOS with Homebrew, NixOS)',
      ...
      questions: [
        {
          ...
          options: [
            ...
            '[ is a command that sees its arguments only after expansion and splitting, so the empty variable disappears; [[ is parsed before expansion',
            '[ cannot compare strings at all, only integers',
            '[[ automatically quotes both operands and [ does not support quotes',
          ],
          correct: 1,
...
\`extglob\` (\`shopt -s extglob\`) upgrades globs with grouping and repetition: \`@(dev|staging|prod)\` exactly one of, \`!(*.min.js)\` everything except, \`+(pattern)\` one or more. It is what makes \`case\` and glob-based filtering expressive enough that you rarely need to shell out to grep for a simple match.`,
      questions: [
        ...
        },
        {
          ...
          options: [
            'The substitution is split on every whitespace character and each word is then glob-expanded',
            'cat cannot be used inside command substitution',
            'for loops cannot iterate over more than 1024 items',
      ...
      questions: [
        {
          ...
          options: [
            ...
            'The line\'s status comes from the local builtin, which succeeded, discarding the substitution\'s failure',
            'set -e already converted the failure into an exit',
          ],
          correct: 2,
      ...
      questions: [
        ...
        },
        {
          ...
          options: [
            ...
            'When you want leading tabs stripped from the body',
            'When the body must be literal — no variable or command expansion — such as an embedded script',
        ...
        },
        {
          ...
          options: [
            ...
            'sudo strips write permissions from redirections for safety',
            'The redirection is performed by your unprivileged shell before sudo runs; use echo line | sudo tee /etc/protected.conf',
      ...
      questions: [
        ...
        },
        {
          ...
          options: [
            ...
            '${v:-default} errors under set -u when v is unset',
            'The colon form also substitutes when v is set but empty; without it, only when v is unset',
          ],
          correct: 3,
        ...
        },
        {
          ...
          options: [
            '[*] joins all elements into one word separated by the first IFS character; [@] yields one word per element',
            'They are identical when quoted',
            '[*] returns the element count instead of the contents',
        ...
        },
        {
          ...
          correct: 2,
          explain: '% removes the shortest matching suffix, so only .gz goes. %% would greedily remove .tar.gz, and ## works on prefixes. (The substitution form would also need a pattern anchored at the end.)',
        },
      ],
    },
    {
      ...
      questions: [
        ...
        },
        {
          ...
          options: [
            ...
            'command -v is a portable builtin; which is an external tool with inconsistent behavior and no knowledge of functions or aliases',
            'which cannot be used inside if statements',
            'command -v also verifies the binary\'s version',
          ],
          correct: 1,
