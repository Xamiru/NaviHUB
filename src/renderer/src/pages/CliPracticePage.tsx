...
// spaces — so only Enter drives the control flow.
...
export default function CliPracticePage() {
  ...
  const pool = useMemo(() => practicePool(selected.length > 0 ? selected : null), [selected])
  ...
  if (playing) {
    return (
      <div className="p-6 max-w-3xl mx-auto">
        <Drill
          items={pool}
          settings={{ sheets: selected.length > 0 ? selected : 'all' }}
