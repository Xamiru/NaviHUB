...
**Scanner integrity (2026-09-21).** A scan is an all-or-nothing snapshot: an unreadable directory/archive or an EPUB with no readable spine aborts before `syncChapters`, so a partial filesystem walk can never prune chapter rows or reading history. Missing EPUB manifest targets are dropped rather than persisted as phantom reader pages. A first attach stores the manga/books root only after the walk succeeds, and every stored chapter path is containment-checked against that root before reads. `markProgress` accepts only integer page indexes inside the current chapter, while mokuro's parsed-overlay cache includes the ordered page list as well as the sidecar path/mtime because changing page membership changes every OCR mapping.
