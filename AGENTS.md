...
`npm run test` takes about 2 minutes for 236 main files / ~2,800 tests plus the renderer suite. Run the whole suite — it is short enough that filtering is rarely worth the risk of missing a regression.
...
**Skills** (`.claude/skills/`) encode multi-step rituals: `add-ipc`, `db-change`, `verify`, `ui-preview`, `local-release` and `wrap`. Follow a matching skill when triggered. `ui-preview` and HANDOFF updates remain explicit-user-request only.
