...
- Spotify playlist imports are **public, one-time snapshots** through user-installed spotDL 4.5.2+ (`spotdl.path`; spotDL resolves downloads through YouTube Music with ordinary YouTube as the built-in fallback). Deno is a required readiness check because current yt-dlp extraction can fail without its JavaScript runtime; Settings can invoke spotDL's official `--download-deno` installer. `music_spotify_playlist_item.matched_track_id` stays nullable with `ON DELETE SET NULL`: a vanished local file greys the source row instead of deleting its metadata/order/retry state. Matching is deliberately strict — exact normalized title + exact primary artist component + both durations within 3 seconds; album breaks one unique tie, and missing duration/ambiguity stays unmatched. Downloads disable lyrics, preserve native Opus with bitrate conversion disabled, use spotDL's normal filtered ranking without `--only-verified-results`, and run in 100-track/four-worker chunks with a scan+resolve after every chunk. A failed row may explicitly disable normal filtering for broader matching or store one exact YouTube source in `download_url`; neither fallback is automatic. Optional YouTube Music Premium cookies (`spotdl.cookieFile`, export-sanitized) switch output to native M4A and can expose 256 kbps audio where the account/source provides it. Extra Piped/Bandcamp/SoundCloud fallbacks are opt-in. Metadata-only saves select YouTube to bypass spotDL's unrelated YouTube Music startup probe; a partial album save falls back to the canonical Spotify album URL and the strict post-download scan remains authoritative. Cancellation keeps finished files and Retry selects the unresolved remainder. Do not label a transcoded file as 320 kbps quality: free YouTube Music audio is normally about 128 kbps and a larger MP3 cannot restore source detail. spotDL, yt-dlp, and music scans share the one-at-a-time music-maintenance gate. A Spotify batch holds one unique, re-entrant gate owner across every child and rescan; never release the batch reservation between chunks.
- Artist and album Spotify downloads use a **persistent release catalogue**, not a process-memory inspection. First open searches Apple's public iTunes metadata catalogue, batches release tracklists four at a time inside a 25-second fast-path budget, and persists albums/singles only in `music_spotify_entity_snapshot` / `_release` / `_track`; repeat opens do no network work until Refresh or Forget. A parallel six-second spotDL lookup checks up to three representative tracks only for a non-tied Spotify identity; it never expands the artist and cannot delay an otherwise-ready preview beyond that bound. Ambiguity is an in-app candidate choice and manual Spotify URLs live under Advanced replacement. Fast-provider failure automatically falls back to spotDL with eight metadata workers, visible elapsed/found progress and cancellation. Do not pass `--use-cache-file`: current spotDL explicitly switches that flag to its official Spotify Web API path, whose day-long application rate limit makes the app appear frozen. Multi-minute provider waits are tree-killed and reported immediately. Downloads resolve only selected releases. Current spotDL's default provider does not reliably support album text searches even though its CLI documents `album:` fields, so unresolved iTunes releases rank release-unique/end tracks first, resolve at most three strict artist/title/duration/edition track queries to discover one album ID, then expand only the canonical Spotify album URL. Apple-only terminal `- Single` / `- EP` labels may be ignored, but real edition markers remain strict. Validate artist/title/track overlap, preserve authoritative payloads across same-provider-release refreshes, then run strict matching and the source-preserved Opus chunked pipeline. A partial spotDL album file never replaces the full indexed release: missing metadata is recovered by exact track queries, standard-edition recordings may be reused only after strict track identity while retaining the selected edition's album metadata, and any remaining gap leaves the complete indexed tracklist intact for Retry. Restartable Pause kills the complete spotDL process tree on every platform, scans completed files and resumes unmatched work while holding one re-entrant maintenance owner; Cancel and app quit must release it and clear transient state. A confirmed mismatch is one-shot only, and source IDs auto-link only when all relevant source tracks resolve to one unambiguous local entity.
- Spotify artist/album releases and imported-playlist rows enter the **persistent Music Downloads queue** before acquisition. `music_spotify_download_queue` groups one saved source and `_selection` stores exact release/item IDs; duplicate adds merge, source deletion cascades, and exports wipe both tables. Adding never starts work automatically. `/music/downloads` starts all or one card, preserves user order, retains failures for Retry and completed cards until Clear, and dynamically rechecks strict local matches before every card. Per-track failure text, normal/broader matching policy, and manual YouTube source survive restarts on the source track rows; exports clear playlist overrides and wipe entity snapshots. One `musicDownload` task owns the re-entrant maintenance gate across the mixed run. Pause tree-kills the active spotDL process, scans and persists the card as paused; interrupted `running` rows normalize to paused on startup and never auto-resume. Cancel returns unfinished work to queued without deleting completed audio. Arbitrary yt-dlp links remain immediate and outside this queue.
...
- yt-dlp/spotDL/ffmpeg are user-installed external binaries (settings `ytdlp.path` / `spotdl.path`), never bundled. spotDL's own Deno installer is exposed in Settings, but Deno remains an external tool stored in spotDL's config directory.
