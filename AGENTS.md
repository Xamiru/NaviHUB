...
- Archive Broadcast chrome is data-driven: `lib/adaptiveNav.ts` owns rail-area classification, drawer destinations and contextual Topbar tabs. Do not hardcode route models separately in `Sidebar.tsx` or `Topbar.tsx`; update `tests/adaptiveNav.test.ts` with the model. `sidebar.hidden` filters both chrome surfaces but never kills routes. Optional context sidecars disappear below `lg`, immersive study/transcript panels cap at `44vw`, and Home's pinned cover wall always leads with a real resume point before its configurable widgets.
