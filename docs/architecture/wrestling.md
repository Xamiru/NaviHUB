...
4. **link targets are redirects** — `[[Steve Austin]]`, `[["Stone Cold" Steve Austin]]` and `[[Stone Cold Steve Austin]]` are one person, so `fetchPages` sends `&redirects=1` and every alias is persisted in the generic `wrestling_alias`; without it every career stat forks;
...
**Re-import is authoritative for canonical fields and preserves the personal layer** by reusing existing match ids through a queue per match title rather than by `sort_order` (a dark match inserted at position 1 would otherwise shift every rating by one; the queue also keeps duplicate titles distinct). Matches removed from the source leave ordinary lists and tier boards before deletion, so their FK-less personal references cannot become ghosts.
...
**(f)** a **second import pass fetches wrestler details** (infobox + lead + Commons portrait via `pageImages`, which works for wrestlers and NOT for events) — without it every wrestler page rendered an empty placeholder forever; params are MERGED across the nested `{{Infobox person|module={{Infobox professional wrestler}}}}` pair (hence `findTemplates`' `nested` option — the outer match would otherwise hide the inner box), and `<br>` becomes a newline so multi-valued fields don't run together. Profile fields, honours, and `detail_fetched_at` commit as one transaction. A successful response marks confirmed missing articles checked; a fetch/write failure stops with a resume hint and leaves that batch NULL for retry. `refresh:true` first reopens profiles referenced by the selected promotions, so “Re-fetch everything” refreshes wrestlers as well as events.
...
**(i)** **a results cell does not end at the last wrestler** — it carries a tail (`… defeated [[Becky Lynch]] by [[pinfall]]`, `to win the [[WWE Championship]]`, `in a [[Ladder match]]`) that was being read as participants; because the method is a WIKILINK it looked exactly like a name, so "pinfall" became a wrestler row with its own page and match count. `splitTail` cuts at the first top-level tail marker and `method` (new column → **ensureColumn**, the table predates it) keeps it; `pruneOrphanWrestlers` runs at the end of an import to sweep rows a re-import stops referencing while preserving wrestlers the user favorited or placed in an ordinary list or tier board. The fixtures had no tail at all, which is exactly why 28 green parser tests sat on top of this — every tail test is now a verbatim modern WWE cell. Sides also join as "A, B and C" (never `&`) and the row says **def.** between them instead of relying on colour.
